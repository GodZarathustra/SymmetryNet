...
# proj_dir = '/home/demian/densefusion_rot/'
...
class PoseDataset(data.Dataset):
    def __init__(self, mode, num_pt, add_noise, root, noise_trans, refine):
        if mode == 'train':
            ...
            self.path = proj_dir +'datasets/ycb/dataset_config/syn_ablation_train_list.txt'
        elif mode == 'syn_ins':
            self.path = proj_dir +'datasets/ycb/dataset_config/syn_ablation_ins_list.txt'
        elif mode == 'syn_frame':
            self.path = proj_dir +'datasets/ycb/dataset_config/syn_ablation_frame_list.txt'
        elif mode == 'syn_class':
        ...
        self.symmetry_obj_idx = [12 ,17 ,21 ,4 ,9 ,11]
        self.ref_sym_list = [12 ,17 ,21 ,4 ,9 ,11]
        self.one_sym_list = [12 ,17 ,21 ,4 ,9 ,11]
        self.two_sym_list = []
        self.three_sym_list = []
        self.only_axis_list = []
        self.axis_and_ref_list = []
    ...
    def __getitem__(self, index):
        ...
        # img_ = np.zeros((540,960,3))
        img = img_

        # img = cv2.resize(img_, (640, 480))
        depth = depth_[:, :, 0]
        ...
        # mask_back = ma.getmaskarray(ma.masked_equal(label, 0))  # 得到标注的mask
        ...
        # cloud = np.dot(np.dot(cam_intri,rt),)
        ...
        #     print('shape_rowid=',row_id.shape)
        #     print('rowid=',row_id)
        img_masked = np.zeros((img_masked.shape))
