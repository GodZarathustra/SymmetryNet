...
parser.add_argument('--dataset', type=str, default = 'scan2cad', help='scan2cad or shapenet')
...
def main():
    ...
    if opt.dataset == 'scan2cad':
        ...
        opt.outf = proj_dir+'trained_models/scannet/'  # folder to save trained models
        opt.log_dir = proj_dir+'experiments/logs/scannet/'  # folder to save logs
        opt.repeat_epoch = 1  # number of repeat times for one epoch training
    elif opt.dataset == 'shapenet':
        opt.num_points = 1000
        opt.outf = proj_dir+'trained_models/shapenet'
        opt.log_dir = proj_dir+'experiments/logs/shapenet'
        opt.repeat_epoch = 20
    else:
    ...
    cad_train_dataset = SymDataset_scan2cad('train', opt.num_points, False, opt.dataset_root,proj_dir, opt.noise_trans)
    cad_test_dataset = SymDataset_scan2cad('holdout_scene', opt.num_points, False, opt.dataset_root,proj_dir, 0.0)

    dataloader = torch.utils.data.DataLoader(cad_train_dataset, batch_size=1, shuffle=True, num_workers=opt.workers)
